...
def cadastro_medico(request):
    ...
    if request.method == "GET":
        ...
        valor_consulta = request.POST.get('valor_cunslta')

        
        
        dado_medico = DadosMedicos(
            crm=crm,
            nome=nome,
            cep=cep,
            rua=rua,
...
def abrir_horario(request):
    ...
    if not is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Somente médicos podem abrir')
    ...
    if request.method == "GET":
        ...
        if data_formatada <= datetime.now():
            ...
            return redirect('/medicos/abir_horario')
